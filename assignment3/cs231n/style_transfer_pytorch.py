...
def content_loss(content_weight, content_current, content_original):
    ...
    F = torch.reshape(content_current, (C_l, -1))
    P = torch.reshape(content_original, (C_l, -1))
...
def gram_matrix(features, normalize=True):
    ...
    for i in range(N):
        gram[i] = features[i, :, :].mm(features[i, :, :].T)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    layer_loss, style_loss = 0.0, 0.0
    for i, layer in enumerate(style_layers):
        G = gram_matrix(feats[layer])
        A = style_targets[i]
        layer_loss = style_weights[i] * torch.sum((G - A) ** 2)
...
def tv_loss(img, tv_weight):
    ...
    img = torch.reshape(img, (3, H, W))
    loss += tv_weight * torch.sum((img[:, :, 1:] - img[:, :, :-1]) ** 2) # by W
    loss += tv_weight * torch.sum((img[:, 1:, :] - img[:, :-1, :]) ** 2) # by H
    return loss

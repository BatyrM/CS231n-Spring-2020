...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        softmax = lambda x: np.exp(x) / np.sum(np.exp(x), axis = 1, keepdims = True) # softmax
        ...
        dh = dr.dot(W2.T)
        dW2 = h.T.dot(dr)
        db2 = dr.T.dot(np.ones((N, ), dtype = np.float32))
        dq = dh * (q > 0).astype(int)
        dW1 = X.T.dot(dq)
        db1 = dq.T.dot(np.ones((N, ), dtype = np.float32))
